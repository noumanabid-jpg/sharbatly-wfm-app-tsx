    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      <Card className="col-span-12 shadow-sm">
        <CardContent className="p-4">
          ...
          <div className="grid gap-2" style={{ gridTemplateColumns: "220px 90px 190px 190px 220px 170px minmax(720px,1fr)" }}>
...
function TimeInput({ value, onChange, disabled }: { value:number, onChange:(m:number)=>void, disabled?:boolean }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <Input type="number" className="h-9 w-16 text-center" min={0} max={23} value={hh} disabled={disabled}
             onChange={e=>{ const v = parseInt((e.target as HTMLInputElement).value||"0",10); onChange(Math.max(0,Math.min(23,v))*60 + mm); }} />
      <span className="text-xs">:</span>
      <Input type="number" className="h-9 w-16 text-center" min={0} max={59} value={mm} disabled={disabled}
